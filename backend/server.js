...
const JWT_SECRET = process.env.JWT_SECRET || 'workpulse-backend-secret-dev-key';
...
app.get('/api/v1/auth/me', authenticateToken, asyncHandler(async (req, res) => {
...
// ---------- Employee Routes ----------
app.get('/api/v1/employees', authenticateToken, authorizeRoles('admin', 'manager'), asyncHandler(async (req, res) => {
  const result = await pool.query(
...
app.get('/api/v1/employees/:id', authenticateToken, authorizeRoles('admin', 'manager', 'employee'), asyncHandler(async (req, res) => {
  const { id } = req.params;
  const result = await pool.query(
    `SELECT id, first_name, last_name, email, phone, department, position, hire_date, salary,
            residence_permit_end_date, employee_number
...
app.patch('/api/v1/employees/:id', authenticateToken, authorizeRoles('admin', 'manager'), asyncHandler(async (req, res) => {
  const { id } = req.params;
...
// ---------- PDF Export Route ----------
app.get('/api/v1/employees/:id/pdf', authenticateToken, authorizeRoles('admin', 'manager', 'employee'), asyncHandler(async (req, res) => {
  const { id } = req.params;
